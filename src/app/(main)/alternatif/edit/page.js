...
import { editAlternatif } from '@/utils/query'
import { Container } from '@/components/Container'
import Link from 'next/link'

const EditAlternatif = (params) => {
  const initialValue = {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch(`/api/edit?id=${params.searchParams.id}`)
...
const EditAlternatifPage = ({ children }) => {
  return (
